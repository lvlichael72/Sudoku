...
def parse_arguments():
    ...
    #create dictionary of keys = argument flag and value  = argument
...
class sudokuGame(object):
    ...
    def __check_block(self,block):
        return set(block)  == set(xrange(1,10))
    ...
    def __check_square(self,row,column):
        return self.__check_block([self.puzzle[r][c] for r in xrange (row * 3,(row +1)*3)
...
class sudokuUI(Frame):
             ...
             The Tkinter UI, responsible for drawing the board and accepting user input.
              ...
              def __draw_victory(self):
                  #create an oval (which will be a circle) that shows __draw_victory
                  x0 = y0 = MARGIN + side * 2
                  x1 = y1 = MARGIN + SIDE * 7
                  self.canvas.create_oval(x0,y0,x1,y1,tags = "victory",fill="dark oxrange",
                  outline = "oxrange")
...
if __name__ == '__main__':
    ...
    with open ('%s.sudoku' % board_name, 'r' ) as board_file:
        game = sudokuGame([line.rstrip() for line in board_file])
        game.start()
